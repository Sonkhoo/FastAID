...
export const getNearestAmbulance = async (latitude: number, longitude: number) => {
    const {data, error} = await supabase.rpc('nearby_ambulance', {
        lat: latitude,
        long: longitude
        });
        ...
        console.log('Nearest ambulance:', data[0]);
        const nearestAmbulance = data[0];
        return {id: nearestAmbulance.id, name: nearestAmbulance.name, location:{
            latitude: nearestAmbulance.lat, longitude: nearestAmbulance.long
        }, is_available: true
    }
}
